...
class Yolov8Node(LifecycleNode):
    ...
    def __init__(self, **kwargs) -> None:
        super().__init__("stop_line_detector", **kwargs)

        self.declare_parameter("image_topic", "/camera_front_down/image_raw")
        self.declare_parameter("model", "stop_line.pt") 
    ...
    def on_configure(self, state: LifecycleState) -> TransitionCallbackReturn:
        ...
        self._debug_pub = self.create_lifecycle_publisher(Image, "debug/image_detections", 10)  # 디버그 이미지 퍼블리셔 추가
    ...
    def on_cleanup(self, state: LifecycleState) -> TransitionCallbackReturn:
        ...
        self.destroy_publisher(self._debug_pub)  # 디버그 퍼블리셔도 파괴
    ...
    def draw_boxes(self, img, results: Results):
        ...
        for box_data in results.boxes:
            ...
            label = f"{self.yolo.names[cls_id]}: {conf:.2f}"
    ...
    def image_cb(self, msg: Image) -> None:
        ...
        if self.enable:
            ...
            # 디버그용 이미지에 바운딩 박스 그리기 및 퍼블리시
            debug_img = self.draw_boxes(cv_image.copy(), results)
